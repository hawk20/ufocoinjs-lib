...
describe('Ufocoin-core', function () {
  // base58EncodeDecode
  ...
  describe('address.toBase58Check', function () {
    ...
    base58KeysValid.forEach(function (f) {
      ...
      var network = params.isTestnet ? ufocoin.networks.testnet : ufocoin.networks.ufocoin
      var version = network[typeMap[params.addrType]]

      it('can export ' + expected, function () {
        assert.strictEqual(ufocoin.address.toBase58Check(hash, version), expected)
      })
    })
  })
  ...
  describe('address.fromBase58Check', function () {
    var allowedNetworks = [
      ufocoin.networks.ufocoin.pubkeyhash,
      ufocoin.networks.ufocoin.scripthash,
      ufocoin.networks.testnet.pubkeyhash,
      ufocoin.networks.testnet.scripthash
    ]
    ...
    base58KeysInvalid.forEach(function (f) {
      ...
      it('throws on ' + string, function () {
        assert.throws(function () {
          var address = ufocoin.address.fromBase58Check(string)
  ...
  describe('ECPair', function () {
    base58KeysValid.forEach(function (f) {
      ...
      var network = params.isTestnet ? ufocoin.networks.testnet : ufocoin.networks.ufocoin
      var keyPair = ufocoin.ECPair.fromWIF(string, network)
  ...
  describe('ECPair.fromWIF', function () {
    var allowedNetworks = [
      ufocoin.networks.ufocoin,
      ufocoin.networks.testnet
    ]
    ...
    base58KeysInvalid.forEach(function (f) {
      ...
      it('throws on ' + string, function () {
        assert.throws(function () {
          ufocoin.ECPair.fromWIF(string, allowedNetworks)
  ...
  describe('Block.fromHex', function () {
    blocksValid.forEach(function (f) {
      it('can parse ' + f.id, function () {
        var block = ufocoin.Block.fromHex(f.hex)
  ...
  describe('Transaction.fromHex', function () {
    txValid.forEach(function (f) {
      ...
      it('can decode ' + fhex, function () {
        var transaction = ufocoin.Transaction.fromHex(fhex)
  ...
  describe('Transaction', function () {
    sigHash.forEach(function (f) {
      ...
      var hashTypes = []
      if ((hashType & 0x1f) === ufocoin.Transaction.SIGHASH_NONE) hashTypes.push('SIGHASH_NONE')
      else if ((hashType & 0x1f) === ufocoin.Transaction.SIGHASH_SINGLE) hashTypes.push('SIGHASH_SINGLE')
      else hashTypes.push('SIGHASH_ALL')
      if (hashType & ufocoin.Transaction.SIGHASH_ANYONECANPAY) hashTypes.push('SIGHASH_ANYONECANPAY')

      var hashTypeName = hashTypes.join(' | ')

      it('should hash ' + txHex.slice(0, 40) + '... (' + hashTypeName + ')', function () {
        var transaction = ufocoin.Transaction.fromHex(txHex)
        assert.strictEqual(transaction.toHex(), txHex)

        var script = Buffer.from(scriptHex, 'hex')
        var scriptChunks = ufocoin.script.decompile(script)
        assert.strictEqual(ufocoin.script.compile(scriptChunks).toString('hex'), scriptHex)
  ...
  describe('ECSignature.parseScriptSignature', function () {
    sigCanonical.forEach(function (hex) {
      ...
      it('can parse ' + hex, function () {
        var parsed = ufocoin.ECSignature.parseScriptSignature(buffer)
    ...
    sigNoncanonical.forEach(function (hex, i) {
      ...
      it('throws on ' + description, function () {
        assert.throws(function () {
          ufocoin.ECSignature.parseScriptSignature(buffer)
