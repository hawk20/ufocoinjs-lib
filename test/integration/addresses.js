...
describe('ufocoinjs-lib (addresses)', function () {
  it('can generate a random address', function () {
    var keyPair = ufocoin.ECPair.makeRandom({ rng: rng })
  ...
  it('can generate an address from a SHA256 hash', function () {
    var hash = ufocoin.crypto.sha256(Buffer.from('correct horse battery staple'))
    var d = bigi.fromBuffer(hash)

    var keyPair = new ufocoin.ECPair(d)
    var address = keyPair.getAddress()
  ...
  it('can import an address via WIF', function () {
    var keyPair = ufocoin.ECPair.fromWIF('Kxr9tQED9H44gCmp6HAdmemAzU3n84H3dGkuWTKvE23JgHMW8gct')
    var address = keyPair.getAddress()
  ...
  it('can generate a 2-of-3 multisig P2SH address', function () {
    ...
    var redeemScript = ufocoin.script.multisig.output.encode(2, pubKeys) // 2 of 3
    var scriptPubKey = ufocoin.script.scriptHash.output.encode(ufocoin.crypto.hash160(redeemScript))
    var address = ufocoin.address.fromOutputScript(scriptPubKey)
  ...
  it('can generate a SegWit address', function () {
    ...
    var scriptPubKey = ufocoin.script.witnessPubKeyHash.output.encode(ufocoin.crypto.hash160(pubKey))
    var address = ufocoin.address.fromOutputScript(scriptPubKey)
  ...
  it('can generate a SegWit address (via P2SH)', function () {
    ...
    var redeemScript = ufocoin.script.witnessPubKeyHash.output.encode(ufocoin.crypto.hash160(pubKey))
    var scriptPubKey = ufocoin.script.scriptHash.output.encode(ufocoin.crypto.hash160(redeemScript))
    var address = ufocoin.address.fromOutputScript(scriptPubKey)
  ...
  it('can generate a SegWit 3-of-4 multisig address', function () {
    ...
    var witnessScript = ufocoin.script.multisig.output.encode(3, pubKeys) // 3 of 4
    var scriptPubKey = ufocoin.script.witnessScriptHash.output.encode(ufocoin.crypto.sha256(witnessScript))
    var address = ufocoin.address.fromOutputScript(scriptPubKey)
  ...
  it('can generate a SegWit 2-of-2 multisig address (via P2SH)', function () {
    ...
    var witnessScript = ufocoin.script.multisig.output.encode(2, pubKeys) // 2 of 2
  ...
  it('can support the retrieval of transactions for an address (3rd party blockchain)', function (done) {
    var keyPair = ufocoin.ECPair.makeRandom()
    var address = keyPair.getAddress()
  ...
  it('can generate a Testnet address', function () {
    ...
    var keyPair = ufocoin.ECPair.makeRandom({ network: testnet, rng: rng })
  ...
  it('can generate a Litecoin address', function () {
    var litecoin = ufocoin.networks.litecoin
    var keyPair = ufocoin.ECPair.makeRandom({ network: litecoin, rng: rng })
    var wif = keyPair.toWIF()
    var address = keyPair.getAddress()
