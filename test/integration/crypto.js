...
describe('ufocoinjs-lib (crypto)', function () {
  it('can recover a private key from duplicate R values', function (done) {
    ...
    mainnet.transactions.get(txIds, function (err, results) {
      ...
      results.forEach(function (tx) {
        transactions[tx.txId] = ufocoin.Transaction.fromHex(tx.txHex)
      })
      ...
      inputs.forEach(function (input) {
        ...
        assert(ufocoin.script.pubKeyHash.input.check(scriptChunks), 'Expected pubKeyHash script')
        ...
        tasks.push(function (callback) {
          mainnet.transactions.get(prevOutTxId, function (err, result) {
            ...
            var prevOut = ufocoin.Transaction.fromHex(result.txHex)
            var prevOutScript = prevOut.outs[prevVout].script

            var scriptSignature = ufocoin.ECSignature.parseScriptSignature(scriptChunks[0])
            var publicKey = ufocoin.ECPair.fromPublicKeyBuffer(scriptChunks[1])
  ...
  it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
    function recoverParent (master, child) {
      ...
      for (var i = 0; i < ufocoin.HDNode.HIGHEST_BIT; ++i) {
        ...
        var Qp = new ufocoin.ECPair(d2).Q
        if (Qp.equals(QP)) break
      }

      var node = new ufocoin.HDNode(new ufocoin.ECPair(d2), master.chainCode, master.network)
    ...
    var master = ufocoin.HDNode.fromSeedBuffer(seed)
