...
describe('ufocoinjs-lib (transactions)', function () {
  it('can create a 1-to-1 Transaction', function () {
    var alice = ufocoin.ECPair.fromWIF('L1uyy5qTuGrVXrmrsvHWHgVzW9kKdrp27wBC7Vs6nZDTF2BRUVwy')
    var txb = new ufocoin.TransactionBuilder()
    ...
    // prepare for broadcast to the Ufocoin network, see "can broadcast a Transaction" below
  ...
  it('can create a 2-to-2 Transaction', function () {
    var alice = ufocoin.ECPair.fromWIF('L1Knwj9W3qK3qMKdTvmg3VfzUs3ij2LETTFhxza9LfD5dngnoLG1')
    var bob = ufocoin.ECPair.fromWIF('KwcN2pT3wnRAurhy7qMczzbkpY5nXMW2ubh696UBc1bcwctTx26z')
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', function (done) {
    ...
    var alice1 = ufocoin.ECPair.makeRandom({ network: testnet })
    var alice2 = ufocoin.ECPair.makeRandom({ network: testnet })
    var aliceChange = ufocoin.ECPair.makeRandom({ rng: rng, network: testnet })
    ...
    testnetUtils.faucetMany([
      ...
      var txb = new ufocoin.TransactionBuilder(testnet)
      ...
      // build and broadcast to the Ufocoin Testnet network
      dhttp({
        method: 'POST',
...
//          url: 'http://tbtc.blockr.io/api/v1/tx/push',
      ...
      // to build and broadcast to the actual Ufocoin network, see https://github.com/ufocoinjs/ufocoinjs-lib/issues/839
    })
  })
  ...
  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', function (done) {
    ...
    var keyPair = ufocoin.ECPair.makeRandom({ network: testnet })
    ...
    testnetUtils.faucet(address, 5e4, function (err, unspent) {
      ...
      var txb = new ufocoin.TransactionBuilder(testnet)
      var data = Buffer.from('ufocoinjs-lib', 'utf8')
      var dataScript = ufocoin.script.nullData.output.encode(data)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a 2-of-4 P2SH(multisig) input', function (done) {
    ...
    ].map(function (wif) { return ufocoin.ECPair.fromWIF(wif, testnet) })
    var pubKeys = keyPairs.map(function (x) { return x.getPublicKeyBuffer() })

    var redeemScript = ufocoin.script.multisig.output.encode(2, pubKeys)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a SegWit P2SH(P2WPKH) input', function (done) {
    ...
    var keyPair = ufocoin.ECPair.fromWIF('cMahea7zqjxrtgAbB7LSGbcQUr1uX1ojuat9jZodMN87JcbXMTcA', testnet)
    var pubKey = keyPair.getPublicKeyBuffer()
    var pubKeyHash = ufocoin.crypto.hash160(pubKey)

    var redeemScript = ufocoin.script.witnessPubKeyHash.output.encode(pubKeyHash)
    var redeemScriptHash = ufocoin.crypto.hash160(redeemScript)

    var scriptPubKey = ufocoin.script.scriptHash.output.encode(redeemScriptHash)
    var address = ufocoin.address.fromOutputScript(scriptPubKey, testnet)
  ...
  it('can create (and broadcast via 3PBP) a Transaction with a SegWit 3-of-4 P2SH(P2WSH(multisig)) input', function (done) {
    ...
    var witnessScript = ufocoin.script.multisig.output.encode(3, pubKeys)
    var redeemScript = ufocoin.script.witnessScriptHash.output.encode(ufocoin.crypto.sha256(witnessScript))
    var scriptPubKey = ufocoin.script.scriptHash.output.encode(ufocoin.crypto.hash160(redeemScript))
    var address = ufocoin.address.fromOutputScript(scriptPubKey, testnet)
  ...
  it('can verify Transaction signatures', function () {
    ...
    ].map(function (q) { return ufocoin.ECPair.fromPublicKeyBuffer(Buffer.from(q, 'hex')) })

    var tx = ufocoin.Transaction.fromHex(txHex)

    tx.ins.forEach(function (input, i) {
      var keyPair = keyPairs[i]
      var prevOutScript = ufocoin.address.toOutputScript(keyPair.getAddress())
      var scriptSig = ufocoin.script.pubKeyHash.input.decode(input.script)
      var ss = ufocoin.ECSignature.parseScriptSignature(scriptSig.signature)
