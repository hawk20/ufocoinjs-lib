...
describe('ufocoinjs-lib (BIP32)', function () {
  it('can import a BIP32 testnet xpriv and export to WIF', function () {
    ...
    var node = ufocoin.HDNode.fromBase58(xpriv, ufocoin.networks.testnet)
  ...
  it('can export a BIP32 xpriv, then import it', function () {
    ...
    var node = ufocoin.HDNode.fromSeedBuffer(seed)
    var string = node.toBase58()
    var restored = ufocoin.HDNode.fromBase58(string)
  ...
  it('can create a BIP32, ufocoin, account 0, external address', function () {
    var path = "m/0'/0/0"
    var root = ufocoin.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, ufocoin, account 0, external address', function () {
    var root = ufocoin.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP49, ufocoin testnet, account 0, external address', function () {
    var mnemonic = 'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about'
    var seed = bip39.mnemonicToSeed(mnemonic)
    var root = ufocoin.HDNode.fromSeedBuffer(seed)

    var path = "m/49'/1'/0'/0/0"
    var child = root.derivePath(path)

    var keyhash = ufocoin.crypto.hash160(child.getPublicKeyBuffer())
    var scriptSig = ufocoin.script.witnessPubKeyHash.output.encode(keyhash)
    var addressBytes = ufocoin.crypto.hash160(scriptSig)
    var outputScript = ufocoin.script.scriptHash.output.encode(addressBytes)
    var address = ufocoin.address.fromOutputScript(outputScript, ufocoin.networks.testnet)
