...
function stealthSend (e, Q) {
  ...
  var c = bigi.fromBuffer(ufocoin.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new ufocoin.ECPair(null, Q.add(cG))
...
function stealthReceive (d, eG) {
  ...
  var c = bigi.fromBuffer(ufocoin.crypto.sha256(eQ.getEncoded()))
  var v = new ufocoin.ECPair(d.add(c).mod(n))
...
function stealthRecoverLeaked (v, e, Q) {
  var eQ = Q.multiply(e) // shared secret
  var c = bigi.fromBuffer(ufocoin.crypto.sha256(eQ.getEncoded()))
  var d = new ufocoin.ECPair(v.subtract(c).mod(n))
...
function stealthDualSend (e, R, Q) {
  ...
  var c = bigi.fromBuffer(ufocoin.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new ufocoin.ECPair(null, R.add(cG))
...
function stealthDualReceive (d, r, eG) {
  var eQ = eG.multiply(d) // shared secret
  var c = bigi.fromBuffer(ufocoin.crypto.sha256(eQ.getEncoded()))
  var v = new ufocoin.ECPair(r.add(c).mod(n))

  return v
}

describe('ufocoinjs-lib (crypto)', function () {
  it('can generate a single-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    var recipient = ufocoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var nonce = ufocoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a single-key stealth address (randomly)', function () {
    var recipient = ufocoin.ECPair.makeRandom() // private to recipient
    var nonce = ufocoin.ECPair.makeRandom() // private to sender
  ...
  it('can generate a dual-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    var recipient = ufocoin.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var scan = ufocoin.ECPair.fromWIF('L5DkCk3xLLoGKncqKsWQTdaPSR4V8gzc14WVghysQGkdryRudjBM') // private to scanner/recipient
    var nonce = ufocoin.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a dual-key stealth address (randomly)', function () {
    var recipient = ufocoin.ECPair.makeRandom() // private to recipient
    var scan = ufocoin.ECPair.makeRandom() // private to scanner/recipient
    var nonce = ufocoin.ECPair.makeRandom() // private to sender
