...
describe('ufocoinjs-lib (blocks)', function () {
  it('can extract a height from a CoinBase transaction', function () {
    ...
    let tx = ufocoin.Transaction.fromHex(txHex)

    assert.strictEqual(tx.ins.length, 1)
    let script = tx.ins[0].script
//  ufocoin.script.decompile(script) // returns [] :(

    assert.strictEqual(script[0], 0x03)
    let heightBuffer = script.slice(1, 4)
    let height = ufocoin.script.number.decode(heightBuffer)
